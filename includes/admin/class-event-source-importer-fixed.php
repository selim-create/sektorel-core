...
class Sektorel_Event_Source_Importer_Fixed {

    const NONCE_ACTION      = 'sektorel_event_source_import';
    const BATCH_SIZE        = 25;
    const MAX_FILE_SIZE     = 5242880; // 5 MB.
    const MAX_XML_ENTRY_SIZE = 10485760; // 10 MB uncompressed XML entry.
    ...
    public static function render_page() {
        ...
        <div class="wrap">
            ...
            <p>Excel veya CSV kaynak listenizi yükleyin. Sistem dosyayı doğrular ve kayıtları <?php echo esc_html( self::BATCH_SIZE ); ?>'erli paketlerle işler.</p>

            <div class="card" style="max-width:900px;padding:22px;margin-top:20px;">
                <h2 style="margin-top:0;">Kaynak Listesi</h2>
                <p class="description">Beklenen kolonlar: <strong>NO</strong>, <strong>Etkinlik İsmi</strong>, <strong>Web Sitesi</strong>, <strong>Türü</strong>. Başlık satırı ilk 20 satır içinde otomatik bulunur. XLSX ve CSV desteklenir.</p>
        ...
        jQuery(function($) {
            ...
            function importNextBatch() {
                ...
                $.post(ajaxurl, {
                    ...
                    if (!response || !response.success) {
                        stopWithError((response && response.data && response.data.message) ? response.data.message : 'Batch işlenemedi.');
                        return;
                    }
                    ...
                    stopWithError('Sunucu isteği başarısız oldu. İşlem durduruldu.');
                });
            }
            ...
            $('#sektorel-source-import-start').on('click', function() {
                ...
                $.ajax({
                    ...
                    if (!response || !response.success) {
                        stopWithError((response && response.data && response.data.message) ? response.data.message : 'Dosya hazırlanamadı.');
                        return;
                    }
                    ...
                    var message = 'Dosya yükleme isteği başarısız oldu.';
    ...
    private static function normalize_rows( $rows ) {
        ...
        for ( $row_index = 0; $row_index < $scan_limit; $row_index++ ) {
            ...
            $candidate_title = self::find_header_index( $headers, array( 'etkinlik ismi', 'etkinlik adı', 'etkinlik adi', 'kaynak', 'başlık', 'baslik' ) );
