...
/**
 * Admin-triggered RSS/XML ingestion for the Content Source Engine.
 * No AI, draft creation or automatic publishing in this phase.
 */
class Sektorel_Content_Source_Scanner {
    ...
    public static function row_actions( $actions, $post ) {
        ...
        $actions['sektorel_content_scan'] = '<a href="' . esc_url( wp_nonce_url(
    ...
    public static function admin_notices() {
        ...
        if ( $scan ) {
            $message = 'ok' === $scan
                ? sprintf(
                    'Tarama tamamlandı. Yeni: %d, mevcut: %d, duplicate: %d.',
                    isset( $_GET['created'] ) ? absint( $_GET['created'] ) : 0,
                    isset( $_GET['existing'] ) ? absint( $_GET['existing'] ) : 0,
                    isset( $_GET['duplicates'] ) ? absint( $_GET['duplicates'] ) : 0
                )
                : 'Kaynak taranamadı. Kaynak ayarlarındaki Son Hata alanını kontrol edin.';
            echo '<div class="notice ' . ( 'ok' === $scan ? 'notice-success' : 'notice-error' ) . ' is-dismissible"><p>' . esc_html( $message ) . '</p></div>';
        }

        if ( 'ok' === ( isset( $_GET['sektorel_content_seed'] ) ? sanitize_key( wp_unslash( $_GET['sektorel_content_seed'] ) ) : '' ) ) {
            echo '<div class="notice notice-success is-dismissible"><p>' . esc_html( sprintf(
                'Resmî başlangıç kaynakları hazırlandı. Yeni: %d, güncellenen: %d.',
                isset( $_GET['seed_created'] ) ? absint( $_GET['seed_created'] ) : 0,
                isset( $_GET['seed_updated'] ) ? absint( $_GET['seed_updated'] ) : 0
            ) ) . '</p></div>';
        }
    }
    ...
    public static function ajax_prepare_scans() {
        ...
        $ids = get_posts( array(
            ...
            'no_found_rows'  => true,
            'meta_query'     => array(
                array( 'key' => 'enabled', 'value' => '1', 'compare' => '=' ),
            ),
        ) );
    ...
    public static function scan_source( $source_id ) {
        ...
        if ( ! $feed_url || ! self::is_safe_public_url( $feed_url ) ) {
            return self::source_error( $source_id, 'Feed URL eksik, geçersiz veya private ağ hedefi.', 'unsafe_feed_url' );
        }
        ...
        $response = wp_safe_remote_get( $feed_url, array(
            ...
            'headers'             => array( 'Accept' => 'application/rss+xml,application/atom+xml,application/xml,text/xml;q=0.9,*/*;q=0.3' ),
        ) );
        ...
        $adapter    = sanitize_key( (string) get_post_meta( $source_id, 'adapter', true ) ) ?: 'rss_generic';
        $role       = sanitize_key( (string) get_post_meta( $source_id, 'role', true ) ) ?: 'official';
        $trust      = sanitize_key( (string) get_post_meta( $source_id, 'trust_level', true ) ) ?: 'medium';
        $language   = sanitize_key( (string) get_post_meta( $source_id, 'language', true ) ) ?: 'tr';
        $fetched_at = gmdate( 'c' );

        foreach ( $items as $item ) {
            $already_exists = self::candidate_exists( $source_key, $item['source_item_key'] );
            $result = Sektorel_Content_Candidates::upsert( $source_id, $source_key, $item['source_item_key'], array(
                'source_url'         => $item['url'],
    ...
    public static function seed_url() {
        return wp_nonce_url( admin_url( 'admin-post.php?action=sektorel_content_seed_defaults' ), 'sektorel_content_seed_defaults' );
    }
    ...
    private static function default_sources() {
        return array(
            array(
                ...
                'meta'       => array(
                    'source_key' => 'tcmb_press', 'base_url' => 'https://www.tcmb.gov.tr/',
                    'feed_url' => 'https://www.tcmb.gov.tr/wps/wcm/connect/TR/TCMB%2BTR/Bottom%2BMenu/Diger/RSS/Basin%2BDuyurulari',
                    'source_type' => 'rss', 'adapter' => 'tcmb_rss', 'role' => 'official', 'trust_level' => 'high',
                    'language' => 'tr', 'category_slugs' => 'ekonomi-piyasalar,finans-bankacilik',
                    'scan_interval' => 'manual', 'enabled' => '1', 'ai_enabled' => '0',
                    'max_items_per_scan' => '25', 'max_ai_items_daily' => '10',
                ),
            ),
            array(
                ...
                'title'      => 'TOBB — Haberler',
                'meta'       => array(
                    'source_key' => 'tobb_news', 'base_url' => 'https://www.tobb.org.tr/',
                    'feed_url' => 'http://www.tobb.org.tr/Sayfalar/RssFeeder.php?List=Haberler',
                    'source_type' => 'rss', 'adapter' => 'rss_generic', 'role' => 'official', 'trust_level' => 'high',
                    'language' => 'tr', 'category_slugs' => 'sirketler-yatirimlar,kobi-girisimcilik,sanayi-uretim',
                    'scan_interval' => 'manual', 'enabled' => '1', 'ai_enabled' => '0',
                    'max_items_per_scan' => '25', 'max_ai_items_daily' => '10',
                ),
            ),
            array(
                ...
                'meta'       => array(
                    'source_key' => 'tobb_announcements', 'base_url' => 'https://www.tobb.org.tr/',
                    'feed_url' => 'http://www.tobb.org.tr/Sayfalar/RssFeeder.php?List=DuyurularListesi',
                    'source_type' => 'rss', 'adapter' => 'rss_generic', 'role' => 'official', 'trust_level' => 'high',
                    'language' => 'tr', 'category_slugs' => 'mevzuat-tesvikler,kobi-girisimcilik,dis-ticaret-ihracat',
                    'scan_interval' => 'manual', 'enabled' => '1', 'ai_enabled' => '0',
                    'max_items_per_scan' => '25', 'max_ai_items_daily' => '10',
                ),
            ),
    ...
    private static function candidate_exists( $source_key, $source_item_key ) {
        ...
        return (bool) $wpdb->get_var( $wpdb->prepare(
            ...
            $source_key,
            mb_substr( $source_item_key, 0, 191 )
        ) );
    }
